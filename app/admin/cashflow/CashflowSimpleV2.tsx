...
type Settlement = { from: Person; to: Person; amount: number } | null;
...
function entryPeople(entry: Entry) {
  ...
  if (entry.entry_type === "settlement") return `${personLabel(entry.from_person)} → ${personLabel(entry.to_person)}`;
  return `Paid by ${personLabel(entry.paid_by)}`;
}
...
export default function CashflowSimpleV2() {
  const [summary, setSummary] = useState<Summary | null>(null);
  const [entries, setEntries] = useState<Entry[]>([]);
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [savingPayerId, setSavingPayerId] = useState("");
  ...
  async function load() {
    ...
    try {
      ...
      if (!response.ok || !result.success || !result.summary) throw new Error(result.message || "Could not load cash flow.");
  ...
  async function changeExpensePayer(entry: Entry, nextPaidBy: Person) {
    if (entry.entry_type !== "expense") return;
    setSavingPayerId(entry.id);
    setError("");
    setNotice("");

    const oldPaidBy = entry.paid_by;
    setEntries((current) => current.map((item) => item.id === entry.id ? { ...item, paid_by: nextPaidBy } : item));
    ...
    try {
      ...
      const result = (await response.json()) as ApiResult;
      if (!response.ok || !result.success) throw new Error(result.message || "Could not change payer.");
      setNotice(`${entry.category}: paid by ${personLabel(nextPaidBy)}.`);
      await load();
    } catch (payerError) {
      setEntries((current) => current.map((item) => item.id === entry.id ? { ...item, paid_by: oldPaidBy } : item));
      setError(payerError instanceof Error ? payerError.message : "Could not change payer.");
    } finally {
      setSavingPayerId("");
    }
  }
          ...
          <>
            ...
            <section className="mt-4 rounded-[30px] border border-violet-400/20 bg-violet-500/[0.07] p-6 sm:flex sm:items-center sm:justify-between sm:gap-6">
              <div>
                ...
                {settlement ? (
                  <><p className="mt-2 text-2xl font-black sm:text-3xl">{personLabel(settlement.from)} → {personLabel(settlement.to)}</p><p className="mt-1 text-4xl font-black text-violet-100">{money(settlement.amount)}</p></>
            ...
            <section className="mt-4 grid gap-3 md:grid-cols-2">
              <article className="rounded-[28px] border border-white/10 bg-white/[0.035] p-5">
                <h2 className="text-xl font-black">Me</h2>
              ...
              <article className="rounded-[28px] border border-white/10 bg-white/[0.035] p-5">
                <h2 className="text-xl font-black">Ahmed Samy</h2>
                <div className="mt-4 grid grid-cols-3 gap-2 text-center">
            ...
            <section className="mt-5">
              ...
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                <button type="button" onClick={() => openQuick("expense")} className="rounded-[24px] border border-red-400/15 bg-red-500/[0.07] p-5 text-left"><span className="text-2xl font-black">−</span><p className="mt-2 text-sm font-black">Add Expense</p></button>
                <button type="button" onClick={() => openQuick("income")} className="rounded-[24px] border border-emerald-400/15 bg-emerald-500/[0.07] p-5 text-left"><span className="text-2xl font-black">+</span><p className="mt-2 text-sm font-black">Add Income</p></button>
                <button type="button" onClick={() => openQuick("capital")} className="rounded-[24px] border border-blue-400/15 bg-blue-500/[0.07] p-5 text-left"><span className="text-2xl font-black">↥</span><p className="mt-2 text-sm font-black">Add Capital</p></button>
                <Link href="/admin/cashflow/transfer" className="rounded-[24px] border border-violet-400/15 bg-violet-500/[0.07] p-5 text-left"><span className="text-2xl font-black">↔</span><p className="mt-2 text-sm font-black">Transfer</p></Link>
              </div>
            </section>

            {quickType && (
              <form onSubmit={saveQuick} className="mt-4 rounded-[30px] border border-white/10 bg-[#0d0d0d] p-5 sm:p-6">
                <div className="flex items-center justify-between gap-4"><h2 className="text-xl font-black">{quickType === "expense" ? "Add Expense" : quickType === "income" ? "Add Income" : "Add Capital"}</h2><button type="button" onClick={() => setQuickType(null)} className="rounded-full border border-white/10 px-3 py-1.5 text-xs font-black text-white/50">Close</button></div>
                ...
                {quickType !== "capital" && <label className="mt-3 block text-xs font-black uppercase tracking-[0.12em] text-white/40">Category<select value={category} onChange={(event) => setCategory(event.target.value)} className="mt-2 w-full rounded-2xl border border-white/10 bg-[#151515] px-4 py-4 text-white outline-none">{(quickType === "expense" ? expenseCategories : incomeCategories).map((item) => <option key={item} value={item}>{item}</option>)}</select></label>}
                <label className="mt-3 block text-xs font-black uppercase tracking-[0.12em] text-white/40">Note <span className="normal-case tracking-normal text-white/25">optional</span><input value={note} onChange={(event) => setNote(event.target.value)} maxLength={300} placeholder="Short note" className="mt-2 w-full rounded-2xl border border-white/10 bg-[#151515] px-4 py-4 text-white outline-none placeholder:text-white/20" /></label>
            ...
            <section className="mt-7 rounded-[30px] border border-white/10 bg-white/[0.03] p-5 sm:p-6">
              ...
              <div className="mt-4 divide-y divide-white/8">
                {recent.length === 0 ? <p className="py-6 text-center text-sm text-white/35">No activity yet.</p> : recent.map((entry) => (
                  <div key={entry.id} className="flex flex-col gap-3 py-4 sm:flex-row sm:items-center">
                    <div className="flex min-w-0 flex-1 items-center gap-3">
                      ...
                      <div className="min-w-0 flex-1"><p className="truncate text-sm font-black">{entry.category}</p><p className="mt-0.5 truncate text-xs text-white/35">{entry.entry_type === "expense" ? entry.entry_date : `${entryPeople(entry)} · ${entry.entry_date}`}</p></div>
                    ...
                    {entry.entry_type === "expense" && (
                      <label className="flex items-center justify-between gap-3 sm:w-[210px] sm:justify-end">
                        <span className="text-[10px] font-black uppercase tracking-[0.12em] text-white/30">Paid by</span>
                        <select
                          value={entry.paid_by || ""}
                          disabled={savingPayerId === entry.id}
                          onChange={(event) => {
                            const next = event.target.value as Person;
                            if (next === "me" || next === "ahmed_samy") void changeExpensePayer(entry, next);
                          }}
                          className="min-w-[130px] rounded-xl border border-white/10 bg-[#111] px-3 py-2 text-xs font-black text-white outline-none disabled:opacity-50"
                        >
