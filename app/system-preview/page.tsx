...
const modules = [
  {
    ...
    copy: "Products, variants, stock states, pricing, cart, wishlist, discount codes, promotional banners, delivery pricing and a complete checkout journey.",
  ...
  },
  {
    ...
    copy: "A post-purchase layer with authentication, saved addresses, synced wishlist, order history, reorder, cancellation, returns and verified reviews.",
  ...
  },
  {
    ...
    items: ["Email / SMS identity verification", "Configurable order journeys", "Bosta-ready dispatch workflow", "ETA and courier tracking support"],
  },
  {
    label: "04 · Control",
    title: "Admin operating system",
    copy: "One command centre for daily operations, launch setup and advanced controls without exposing internal complexity to customers.",
...
const demoLinks = [
  ...
  { href: "/admin/buyer-preview", label: "Read-only admin demo", detail: "Synthetic data with no production write connection" },
];

const handoverItems = [
  "Next.js 16 source-code snapshot",
  "Supabase schema and ordered migrations",
  "Environment-variable template without production secrets",
  "White-label Brand & SEO control centre",
  "Six-step Setup & Launch workflow",
  "Buyer handover and acceptance checklist",
];
    ...
    <main className="min-h-screen overflow-hidden bg-[#07080a] text-white">
      <section className="relative border-b border-white/10 px-5 py-20 sm:px-8 sm:py-28">
        ...
        <div className="relative mx-auto max-w-6xl">
          <div className="flex flex-wrap items-center gap-2">
            <span className="rounded-full border border-blue-300/20 bg-blue-300/[0.07] px-4 py-2 text-[10px] font-black uppercase tracking-[0.2em] text-blue-100/75">Private buyer preview</span>
            <span className="rounded-full border border-emerald-300/20 bg-emerald-300/[0.07] px-4 py-2 text-[10px] font-black uppercase tracking-[0.2em] text-emerald-100/75">Production deployed</span>
          </div>

          <div className="mt-8 grid items-end gap-10 lg:grid-cols-[1fr_0.55fr]">
            <div>
              <h1 className="max-w-4xl text-5xl font-black leading-[0.96] tracking-[-0.055em] sm:text-7xl">A ready-to-brand commerce operating system.</h1>
              ...
              <div className="mt-9 flex flex-wrap gap-3">
                <Link href="/admin/buyer-preview" className="rounded-2xl bg-white px-6 py-3.5 text-sm font-black text-black transition hover:bg-blue-50">Open safe admin demo</Link>
                <Link href="/" className="rounded-2xl border border-white/13 bg-white/[0.045] px-6 py-3.5 text-sm font-black text-white/76 transition hover:bg-white/[0.08]">Open live store</Link>
                <Link href="/track-order" className="rounded-2xl border border-white/13 bg-white/[0.045] px-6 py-3.5 text-sm font-black text-white/76 transition hover:bg-white/[0.08]">Test secure tracking</Link>
              </div>
            </div>

            <aside className="rounded-[30px] border border-emerald-300/15 bg-[linear-gradient(145deg,rgba(16,185,129,0.09),rgba(255,255,255,0.025))] p-6 shadow-2xl shadow-black/25">
              <p className="text-[10px] font-black uppercase tracking-[0.18em] text-emerald-100/45">Current direct offer</p>
              <p className="mt-3 text-5xl font-black tracking-[-0.055em]">$750</p>
              <p className="mt-2 text-xs font-black uppercase tracking-[0.12em] text-white/35">White-label source package</p>
              <p className="mt-5 text-sm leading-6 text-white/45">A clean buyer-controlled deployment with source, migrations, rebranding workflow and handover documentation. Final rights and support scope are confirmed in the signed commercial agreement.</p>
            </aside>
          </div>
        </div>
      </section>
              ...
              <article key={module.title} className="rounded-[30px] border border-white/9 bg-white/[0.025] p-6 sm:p-8">
                ...
                <ul className="mt-6 grid gap-2 sm:grid-cols-2">
                  {module.items.map((item) => (
                    <li key={item} className="rounded-xl border border-white/7 bg-black/20 px-3 py-3 text-xs font-bold text-white/58"><span className="mr-2 text-emerald-300">✓</span>{item}</li>
                  ))}
                </ul>
              </article>
            ))}
        ...
        <div className="mx-auto max-w-6xl">
          <div className="grid gap-10 lg:grid-cols-[0.62fr_1fr]">
            <div>
              ...
              <p className="mt-5 text-sm font-medium leading-7 text-white/42">These links open real customer-facing surfaces or the synthetic buyer admin demo. Production admin data remains private.</p>
            </div>
            <div className="grid gap-3 sm:grid-cols-2">
              {demoLinks.map((item) => (
                <Link key={item.href} href={item.href} className="group rounded-2xl border border-white/9 bg-black/20 p-5 transition hover:border-blue-300/25 hover:bg-blue-300/[0.045]">
                  <div className="flex items-center justify-between gap-4"><p className="text-sm font-black">{item.label}</p><span className="text-white/25 transition group-hover:translate-x-1 group-hover:text-blue-200">→</span></div>
      ...
      <section className="px-5 py-20 sm:px-8 sm:py-24">
        <div className="mx-auto max-w-6xl rounded-[36px] border border-blue-300/15 bg-[linear-gradient(135deg,rgba(37,99,235,0.13),rgba(255,255,255,0.025))] p-7 sm:p-10">
          <div className="grid gap-9 lg:grid-cols-[1fr_0.72fr]">
            <div>
              ...
              <p className="mt-5 max-w-2xl text-sm font-medium leading-7 text-white/45">The buyer receives a documented source package and launches through their own Vercel, Supabase and optional provider accounts. ORVIX production customer data and production secrets are not part of the handover.</p>
            </div>
      ...
      <section className="border-t border-white/10 px-5 py-20 sm:px-8 sm:py-24">
        <div className="mx-auto grid max-w-6xl gap-8 rounded-[36px] border border-emerald-300/15 bg-emerald-300/[0.045] p-7 sm:p-10 lg:grid-cols-[0.72fr_1fr] lg:items-end">
          <div>
            <p className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-100/55">Current offer</p>
            <p className="mt-4 text-5xl font-black tracking-[-0.055em]">$750</p>
            <p className="mt-3 text-xs font-bold leading-5 text-white/35">White-label source package · buyer-controlled deployment</p>
          </div>
          <div>
            <h2 className="text-3xl font-black tracking-[-0.04em] sm:text-4xl">Launch without rebuilding the commerce stack.</h2>
            <p className="mt-5 max-w-2xl text-sm font-medium leading-7 text-white/45">The package is designed for a serious buyer who wants a working commerce foundation, a safe technical demo and a clean deployment path. Commercial rights, included support and payment terms are confirmed directly with the seller before transfer.</p>
            <div className="mt-7 flex flex-wrap gap-3">
              <Link href="/admin/buyer-preview" className="rounded-2xl bg-white px-6 py-3.5 text-sm font-black text-black transition hover:bg-emerald-50">Open buyer admin demo</Link>
              <Link href="/" className="rounded-2xl border border-white/12 bg-black/20 px-6 py-3.5 text-sm font-black text-white/70">Open live storefront</Link>
            </div>
          </div>
        </div>
      </section>
