...
export default function ProductCheckout(){
  ...
  useEffect(()=>{if(!cityId){setDistricts([]);setDistrictId("");return;}let cancelled=false;(async()=>{try{const r=await fetch(`/api/bosta/locations?cityId=${encodeURIComponent(cityId)}`,{cache:"no-store"});const j=await r.json();if(!r.ok||!j.success)throw new Error(j.message||"Could not load districts.");if(!cancelled)setDistricts(Array.isArray(j.districts)?j.districts:[]);}catch(e){if(!cancelled)setError(e instanceof Error?e.message:"Could not load districts.");}})();return()=>{cancelled=true};},[cityId]);
  ...
  useEffect(()=>setQuantity(q=>Math.min(Math.max(q,1),maxQty)),[maxQty]);
